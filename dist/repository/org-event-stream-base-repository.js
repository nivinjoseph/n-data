...
export class OrgEventStreamBaseRepository extends BaseRepository {
    ...
    async getAll(...ids) {
        ...
        if (ids.isNotEmpty)
            return this.query(`select data from ${this.table} where organization_id = ? and aggregate_id in (${ids.map(() => "?").join(",")});`, this.domainContext.organizationId, ...ids);
        return this.query(`select data from ${this.table} where organization_id = ?;`, this.domainContext.organizationId);
    }
    async get(id) {
        given(id, "id").ensureHasValue().ensureIsString();
        id = id.trim();
        const result = await this.query(`select data from ${this.table} where organization_id = ? and aggregate_id = ?;`, this.domainContext.organizationId, id);
        if (result.length !== 1)
