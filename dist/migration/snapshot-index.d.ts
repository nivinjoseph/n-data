...
export type SerializedShapeOf<V> = [
    V
] extends [DomainObject<object, never>] ? SerializedValue<V> extends infer TData extends object ? [TData] extends [ReadonlyArray<any>] ? Record<never, never> : string extends keyof TData ? Record<never, never> : TData : Record<never, never> : true extends (V extends {
    serialize(): object;
...
type SnapshotLeafPath<T, TDepth extends number = 5> = [TDepth] extends [never] ? never : string extends keyof T ? never : {
    [K in keyof T & string]: K extends `$${string}` ? never : IsAnyOrUnknown<T[K]> extends true ? never : NonNullable<T[K]> extends Function ? never : NonNullable<T[K]> extends SnapshotOpaqueContainer ? never : NonNullable<T[K]> extends ReadonlyArray<any> ? never : NonNullable<T[K]> extends object ? `${K}.${SnapshotLeafPath<SerializedShapeOf<NonNullable<T[K]>>, PreviousDepth[TDepth]>}` : [NonNullable<T[K]>] extends [JsonScalar] ? K : never;
}[keyof T & string];
/**
...
export declare class SnapshotIndex<T> {
     ...
     */
    get casts(): ReadonlyArray<JsonValueType | undefined>;
    /**
