...
- **Paths are type-checked against the *stored* shape, and validated where they are written.** The set is generic over the aggregate's state, so `withPath("stauts")` is a compile error, including for nested keys like `"customer.ctiy"`. One level down, paths follow what `serialize()` actually stores: for a member with a typed `serialize()` (an n-domain >= 4.0.1 `DomainObject`), a segment compiles only if a getter of that name is serialized — a derived, undecorated getter is a compile error rather than an always-null index — while a *bare* `Serializable` member (untyped `serialize()`) offers no nested paths at all, since there is nothing to check them against. Still unchecked: an explicit `@serialize("customKey")` rename, which is invisible to the type and remains offered under its TypeScript name — the one silent case left — and the stored `$typename`, which no path (typed or raw) can address. A malformed path, a bad type or a repeated path throws at the declaration, not when the table is created. Nesting is checked up to six segments deep; past that, and for a computed or `$`-prefixed key, `SnapshotIndex.forRawPath` is the way through.
...
- **Only arrays of scalars, or of flat scalar records, are offered — judged by the *stored* element shape.** An element with a typed `serialize()` (an n-domain >= 4.0.1 `DomainObject`) is judged by, and matched against, its serialized record: when that record is flat the array is indexable, and the `$typename` each stored element also carries never blocks a match, since `@>` is subset matching. An array of *untyped* `Serializable` is a compile error, because there is nothing to check a match document against; and an explicit `@serialize("customKey")` rename stays invisible to the type, so a document built from the TypeScript name would match nothing, silently. Plain object literals are safe, since nested plain objects are copied through `JSON.parse(JSON.stringify(...))`. `forRawPath` is the door for everything else, where the caller owns knowing the stored shape — and a raw match document may name `$typename`, which is the escape hatch for filtering a polymorphic element by its stored type.
