...
A read that is genuinely meant to span tenants — and only that — goes through `queryAcrossOrganizations`, which takes a whole statement and adds nothing to it. It is named for its consequence so the tenant implication is visible at the call site. On the non-org repositories the same escape hatch is called `queryStatement`, since there is no tenant scope to step outside of; it is there for the joins, unions and CTEs the built statement cannot express.
